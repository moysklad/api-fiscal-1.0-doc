...
В [дескрипторе](https://dev.moysklad.ru/doc/api/vendor/1.0/#deskriptor-resheniq) приложения, в блоке `<fiscalApi>`, указать:
   - `endpointBase` - базовый URL на стороне разработчика с имплементацией API
...
## Рекомендации по настройке формата печати чеков

Формат чека, который передается на печать, задается на стороне вендора фискального решения (п.6 Сценарии работы).

Чтобы обеспечить корректную печать чеков рекомендуется:

- уточнить модель подключенного принтера и поддерживаемый им формат бумаги
- проверить и при необходимости скорректировать настройки принтера. Более подробно о подключении Xprinter можно посмотреть в [инструкции](https://support.moysklad.ru/hc/ru/articles/8344440530705-%D0%9F%D0%BE%D0%B4%D0%BA%D0%BB%D1%8E%D1%87%D0%B5%D0%BD%D0%B8%D0%B5-XPrinter?_ga=2.68741523.308955161.1766575500-555252124.1763047279&_gl=1%2A1xrqksw%2A_gcl_au%2AMjA5OTI4NjA5OC4xNzYwMTk4ODkx%2A_ga%2ANTU1MjUyMTI0LjE3NjMwNDcyNzk.%2A_ga_P5SLT3G3F7%2AczE3NjY1Nzc3NDMkbzM1JGcxJHQxNzY2NTc5Mzc2JGoyNiRsMCRoMA..)
- убедиться, что выбран соответствующий формат бумаги в настройках Кассы МойСклад. Поддерживаются следующие размеры:
  - 56 мм
  - 80 мм
  - А4
- в настройках принтера отключить сглаживание, чтобы избежать размытых шрифтов и некорректного отображения элементов чека
